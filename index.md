...
The main function of zoneR is
[`zr_run_zoning_checks()`](https://vibe-lab-gsd.github.io/zoneR/reference/zr_run_zoning_checks.md)
which reads in an OZFS `.bldg`, `.parcel`, and `.zoning` file. It
calculates the zoning requirements for each parcel, checks them against
the building characteristics, and returns a data frame with a row for
each parcel and an “allowed” column containing one of three possible
values.
...
We can then put each of the files into the
[`zr_run_zoning_checks()`](https://vibe-lab-gsd.github.io/zoneR/reference/zr_run_zoning_checks.md)
...
[`zr_run_zoning_checks()`](https://vibe-lab-gsd.github.io/zoneR/reference/zr_run_zoning_checks.md),
but they can be used by themselves if desired. Below are some of the
main variables that the different zoneR functions take in.

- **bldg_data:** The `.bldg` file read in as a list using
  [`rjson::fromJSON()`](https://rdrr.io/pkg/rjson/man/fromJSON.html)

- **parcel_dims:** Created with the
  [`zr_get_parcel_dims()`](https://vibe-lab-gsd.github.io/zoneR/reference/zr_get_parcel_dims.md)
  function. A simple features data frame with all the centroid and
  dimensional data from the `.parcel` file. It contains one row per
  parcel.
...
- **parcel_geo:** Created with the
  ...
  function. A simple features data frame containing the geometry of each
  parcel side without the centroid or dimensional data.

- **zoning_data:** The data in the `.zoning` file read in as a simple
  features data frame using
